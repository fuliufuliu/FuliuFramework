    ...
    public class CPseudocodeParser : IParser
    {
        ...
        private PseudocodeFunc CreatePseudocodeFunc(string sentence)
        {
            ...
            if (sentence.Contains('('))
            {
                ...
                switch (res.funcName)
                {
                    ...
                    case "NewFunc": 
                        ...
                        res.type = "Func"; // 注意这里是大写，此种类型每次循环都要执行
                        res.needCompute = false;
                        break;
                    default: 
            ...
            else
            {
                ...
                else if (float.TryParse(res.funcName, out float resFloat))
                {
                    res.result = resFloat;
                    res.type = "float";
                    res.needCompute = false;
                }
                else if (bool.TryParse(res.funcName, out bool resBool))
                {
                    res.result = resBool;
                    res.type = "bool";
                    res.needCompute = false;
                }
                else if (res.funcName.Contains("\""))
                {
