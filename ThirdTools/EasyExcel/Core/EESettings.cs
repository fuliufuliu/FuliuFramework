...
namespace EasyExcel
{
	...
	public class EESettings : ScriptableObject
	{
		...
		[EEComment(
			"This row of an excel sheet is discription (starting from 0)",
			"指定这一行字段说明（行号从0开始）")]
		[SerializeField]
		private int discriptionRowIndex;
		...
		public int DiscriptionRowIndex
		{
			get { return discriptionRowIndex; }
			set
			{
				if (discriptionRowIndex == value)
					return;
				discriptionRowIndex = value;
				modified = true;
			}
		}
