...
public class InlieText : Text
{
    ...
    protected override void OnPopulateMesh(VertexHelper toFill)
    {
        ...
        for (int i = 0; i < listTagInfor.Count; i++)
        {
            ...
            //计算其uv
            Rect spriteRect = m_spriteAsset.sprites[0].rect;
            for (int j = 0; j < m_spriteAsset.sprites.Length; j++)
            {
                //通过标签的名称去索引spriteAsset里所对应的sprite的名称
                if (listTagInfor[i].name == m_spriteAsset.sprites[j].name)
                    spriteRect = m_spriteAsset.sprites[j].rect;
            }
            Vector2 texSize = new Vector2(m_spriteAsset.mainTexture.width, m_spriteAsset.mainTexture.height);
