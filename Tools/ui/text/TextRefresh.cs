...
public class TextRefresh : MonoBehaviour
{
    ...
    private static TextRefresh m_Instance = null;

    void Awake()
    {
        if (null != m_Instance)
        {
            Debug.LogError("There is only one TextRefresh instance allowed!");
            Destroy(gameObject);
            return;
        }
    ...
    void Start()
    {
        ...
        m_RebuildForFont = fontUpdateTrackerType.GetMethod("RebuildForFont", System.Reflection.BindingFlags.Static);
